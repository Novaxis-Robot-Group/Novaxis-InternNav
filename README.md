...
## Novaxis Real-World Deployment

This repository extends the upstream InternNav project with the Novaxis
real-world inference service, experiment viewer, Upper Agent, speech input,
Experiment QA, HTTP/Zenoh transport tools, and experiment-memory components.

Model weights, API keys, runtime configuration, and experiment logs are not
stored in Git. Put local model weights under `checkpoints/` and keep secrets in
environment variables.

### Quick start: Experiment Viewer on port 8899

Create an environment and install the lightweight viewer dependencies:

```bash
conda create -n novaxis-internnav python=3.10 -y
conda activate novaxis-internnav
pip install -r requirements/realworld_visualizer.txt
```

Start the viewer from the repository root:

```bash
bash scripts/run_experiment_visualizer.sh
```

Equivalent full command:

```bash
python scripts/realworld/experiment_visualizer.py \
  --serve \
  --host 0.0.0.0 \
  --port 8899 \
  --log_dir output/realworld_experiments \
  --runtime_config_path output/realworld_runtime_config.json
```

The server prints `http://0.0.0.0:8899/`. When it runs on a remote machine,
create the tunnel on your local computer and open `http://127.0.0.1:8899/`:

```bash
ssh -L 8899:127.0.0.1:8899 <user>@<server-ip>
```

The viewer creates `output/` automatically. Optional Experiment QA, speech,
YOLO/SAM analysis, local Qwen, and robot-model services require their
corresponding dependencies and model weights; the basic viewer starts without
shipping those large artifacts in this repository.

### Start the InternVLA HTTP model service

Install the full model dependencies described by the upstream documentation,
download a compatible InternVLA-N1 checkpoint, then run from the repository
root (adjust GPU and checkpoint path as needed):

```bash
CUDA_VISIBLE_DEVICES=1 python scripts/realworld/http_internvla_server.py \
  --device cuda:0 \
  --model_path checkpoints/InternVLA-N1-DualVLN \
  --host 0.0.0.0 \
  --port 8848 \
  --experiment_log_dir output/realworld_experiments \
  --runtime_config_path output/realworld_runtime_config.json
```

`CUDA_VISIBLE_DEVICES=1` exposes physical GPU 1 as `cuda:0` inside the
process. Never commit files under `checkpoints/`, `output/`, or `.env`.

### Optional features

```bash
# Full-frame Experiment QA detector
pip install -r requirements/experiment_analysis.txt

# Cross-experiment Upper Agent memory
pip install -r requirements/agent_memory.txt
```

The original InternNav project introduction and model documentation continue
below.
